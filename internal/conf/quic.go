...
func (q *QUIC) setDefaults(role string) {
	if q.MaxIdleTimeout == 0 {
		// Increase idle timeout for high-pressure scenarios
		if role == "server" {
			q.MaxIdleTimeout = 60 // 60 seconds for server
		} else {
			q.MaxIdleTimeout = 30 // 30 seconds for client
		}
	}

	if q.MaxIncomingStreams == 0 {
		if role == "server" {
			q.MaxIncomingStreams = 50000 // Very high limit for servers under pressure
		} else {
			q.MaxIncomingStreams = 5000
		}
	}

	if q.MaxIncomingUniStreams == 0 {
		if role == "server" {
			q.MaxIncomingUniStreams = 50000
		} else {
			q.MaxIncomingUniStreams = 5000
		}
	}

	// Flow control defaults optimized for high bandwidth and many connections
	if q.InitialStreamReceiveWindow == 0 {
		if role == "server" {
			q.InitialStreamReceiveWindow = 10 * 1024 * 1024 // 10 MB for server
		} else {
			q.InitialStreamReceiveWindow = 6 * 1024 * 1024 // 6 MB for client
		}
	}

	if q.MaxStreamReceiveWindow == 0 {
		if role == "server" {
			q.MaxStreamReceiveWindow = 40 * 1024 * 1024 // 40 MB for server
		} else {
			q.MaxStreamReceiveWindow = 24 * 1024 * 1024 // 24 MB for client
		}
	}

	if q.InitialConnectionReceiveWindow == 0 {
		if role == "server" {
			q.InitialConnectionReceiveWindow = 30 * 1024 * 1024 // 30 MB for server
		} else {
			q.InitialConnectionReceiveWindow = 15 * 1024 * 1024 // 15 MB for client
		}
	}

	if q.MaxConnectionReceiveWindow == 0 {
		if role == "server" {
			q.MaxConnectionReceiveWindow = 100 * 1024 * 1024 // 100 MB for server
		} else {
			q.MaxConnectionReceiveWindow = 60 * 1024 * 1024 // 60 MB for client
		}
	}
